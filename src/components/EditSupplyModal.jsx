...
function EditSupplyModal({ onClose, onSave, editData }) {
  const [formData, setFormData] = useState({
    specialty: editData?.specialty || "08.02.01",
    cycle: editData?.cycle || "ОУДБ.01",
    subject: editData?.subject || "Русский язык",
    academicYear: editData?.academicYear || "2025-2026",
    educationLevel: editData?.educationLevel || "11 класс",
    selectedBooks: editData?.selectedBooks || []
  })
  ...
  const [selectedBooks, setSelectedBooks] = useState([...formData.selectedBooks])
  ...
  const handleSubmit = () => {
    ...
    console.log('Изменённые данные записи:', result)
    alert('Запись обновлена (заглушка)')
    onClose()
  }
...
>
  ✕
        ...
        <h2 style={{ fontSize: '24px', margin: '0 0 20px 0', color: '#333' }}>Редактирование записи в картотеку</h2>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {filteredCatalogBooks.map(book => {
                      // Подсветка выбранных книг (галочки уже отмечены)
                      const isSelected = selectedBooks.some(b => b.id === book.id)
                      return (
                        <tr key={book.id} style={{ borderBottom: '1px solid #eee', backgroundColor: isSelected ? 'rgba(54, 54, 255, 0.08)' : 'transparent' }}>
                          <td style={{ padding: '12px' }}>
                            <input type="checkbox" checked={isSelected} onChange={() => toggleBookSelection(book)} />
                           </td>
        ...
        <div style={{ display: 'flex', gap: '16px', marginTop: '30px', flexDirection: 'column' }}>
          <button onClick={handleSubmit} style={{ width: '100%', padding: '12px', backgroundColor: '#3636FF', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}>Сохранить изменения</button>
