...
function AddBook() {
  ...
  const handleSubmit = (e) => {
    ...
    alert('Книга добавлена (заглушка)')
  }

  const handleAddContributorSimple = () => {
    if (contributorName.trim()) {
      setFormData({ ...formData, contributors: [...formData.contributors, { name: contributorName, role: contributorRole }] })
      setContributorName('')
      setContributorRole('')
    }
  }

  const handleAddPublisherSimple = () => {
    if (publisherInput.trim()) {
      setFormData({ ...formData, publishers: [...formData.publishers, publisherInput] })
      setPublisherInput('')
    }
  }
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', backgroundColor: '#f9f9f9', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        />
        <div>
          <h1 style={{ fontSize: '24px', margin: 0, color: '#333', fontWeight: '700' }}>Добавление книги</h1>
